...
if (isset($_GET['acao'])) {
    $id = $_GET['codigo'];
    DAOFactory::getBannersuperiorDAO()->delete($id);
    echo"<meta http-equiv='refresh' content=3;url='bannersuperior.php'>";
}
...
$uploaddir = '../imagem/bannersuperior/';
$nome = rand(00, 9999);
...
if ($action == "add") {
    try {
        ...
        $Bannersuperior->texto1 = $texto1;
        $Bannersuperior->texto2 = $texto2;
        $Bannersuperior->datadecriacao = $datainicial;
        $Bannersuperior->datainicial = $datainicial;
        $Bannersuperior->datafinal = $datafinal;
        $Bannersuperior->imagem = $foto1;
        $id = DAOFactory::getBannersuperiorDAO()->insert($Bannersuperior);
        $transaction->commit();
        echo"<meta http-equiv='refresh' content=3;url='addBannerSuperior.php'>";
    } catch (Exception $exc) {
...
if ($action == "edit") {
    try {
        ...
        $Bannersuperior->datafinal = $datafinal;
        if (trim($foto1) == "") {
            $Bannersuperior->imagem = $_POST['foto1only'];
        } else {
            $Bannersuperior->imagem = $foto1;
        }
        
        DAOFactory::getBannersuperiorDAO()->update($Bannersuperior);
        $transaction->commit();
        echo"<meta http-equiv='refresh' content=3;url='bannersuperior.php'>";
    } catch (Exception $exc) {
        //header('location: login.php?msg=error');
